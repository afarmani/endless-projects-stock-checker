...
import com.endless.projects.stock.checker.dto.bestbuy.Availabilities;
import com.endless.projects.stock.checker.dto.bestbuy.BestBuyDto;
...
public class StockCheckerFunctionTest {
    ...
    public void UrlConnectionTest() throws IOException {
        ...
        // convert input stream into DTO
        ObjectMapper objectMapper = new ObjectMapper();
        BestBuyDto bestBuyDto = objectMapper.readValue(response, BestBuyDto.class);

        Assert.assertTrue(bestBuyDto.getAvailabilities().length>0);

        for (Availabilities availabilities : bestBuyDto.getAvailabilities()) {
            System.out.println("SKU: " + availabilities.getSku() + " pickup: " + availabilities.getPickup().getStatus() + " shipping: " + availabilities.getShipping().getStatus());
        }
    }
}